...
import { Package, DollarSign, ArrowRight, Star, Truck, Shield, Zap } from 'lucide-react';

// Standard promo boxes
const standardBoxes = [
  ...
  { size: 'Large',       price: '$170', emoji: '📦', highlight: false, color: '#163F86', dims: '~18×18×18"' },
  { size: 'Extra Large', price: '$240', emoji: '📦', highlight: false, color: '#0B2E63', dims: '~22×22×22"' },
];

// Custom dimension boxes — corrected & updated list
const customBoxes = [
  { dims: '20×20×20"', price: '$150' },
  { dims: '22×22×22"', price: '$185' },
  { dims: '24×24×24"', price: '$215' },
  { dims: '36×22×22"', price: '$265' },
  { dims: '36×24×24"', price: '$310' },
  { dims: '30×30×30"', price: '$370' },
  { dims: '42×26×29"', price: '$380' },
];

// Barrels — two separate products with different prices
const barrels = [
  { label: 'BARRIL 55 GAL', price: '$280', color: '#163F86' },
  { label: 'BARRIL 77 GAL', price: '$380', color: '#D72638' },
];
...
const advantages = [
  ...
  },
  {
    icon: Package,
    color: '#0B2E63',
    bg: 'rgba(11,46,99,0.08)',
    border: 'rgba(11,46,99,0.15)',
    emoji: '🤝',
  },
  {
    icon: Zap,
    color: '#D72638',
    bg: 'rgba(215,38,56,0.08)',
    border: 'rgba(215,38,56,0.15)',
    emoji: '⚡',
  },
];
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {advantages.map((adv, i) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -4, boxShadow: `0 16px 32px ${adv.bg}` }}
                className="bg-white rounded-2xl p-5 border flex items-center gap-4 transition-all duration-300"
                style={{
                  boxShadow: '0 4px 16px rgba(11,46,99,0.06)',
              ...
              >
                ...
                <div>
                  <div className="font-black text-[#111827] text-sm leading-tight">
                    {t(`adv${i + 1}_title`)}
                  </div>
            ...
            {standardBoxes.map((box, i) => (
              <motion.div
                ...
                className={`relative rounded-2xl p-6 text-center bg-white border transition-all duration-300 cursor-default ${
                  box.highlight ? 'border-[#D72638]/30' : 'border-gray-100'
                }`}
                style={box.highlight
                  ? { boxShadow: '0 8px 32px rgba(215,38,56,0.12)' }
                  : { boxShadow: '0 4px 16px rgba(11,46,99,0.06)' }}
              >
                {box.highlight && (
                  <div
                    className="absolute -top-3 left-1/2 -translate-x-1/2 flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold text-white shadow-lg whitespace-nowrap"
                ...
                <div className="text-3xl font-black" style={{ color: box.color }}>
                  {box.price}
                </div>
              </motion.div>
            ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            ...
            {/* Table header */}
            <div
              className="flex items-center justify-between px-3 py-2 rounded-lg mb-2 text-xs font-bold uppercase tracking-wider text-gray-400"
          ...
          >
            ...
            <div className="bg-white rounded-2xl p-6 sm:p-8 border border-gray-100 flex-1"
              style={{ boxShadow: '0 4px 24px rgba(11,46,99,0.06)' }}
            >
              <h3 className="text-lg font-bold text-[#111827] mb-5 flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-3 mb-5">
                {barrels.map((barrel, i) => (
                  <motion.div
                    ...
                    whileHover={{ y: -4, boxShadow: `0 16px 32px ${barrel.color}25` }}
                    className="relative rounded-2xl p-4 text-center overflow-hidden border transition-all duration-300"
                    style={{
                      background: `${barrel.color}08`,
                      borderColor: `${barrel.color}25`,
                      boxShadow: `0 4px 16px ${barrel.color}10`,
                    }}
                  >
                    {/* Accent top bar */}
                    <div
                      className="absolute top-0 left-0 right-0 h-1 rounded-t-2xl"
                      style={{ background: barrel.color }}
                    />
                    <div className="text-3xl mb-2">🛢️</div>
                    <div
                      className="text-xs font-bold uppercase tracking-wider mb-2"
                      style={{ color: barrel.color }}
                    >
                      {barrel.label}
                    </div>
                    <div
              ...
              <div className="flex flex-wrap gap-2">
                {['🇳🇮 Nicaragua', '🇻🇪 Venezuela'].map(dest => (
                  <span
                    ...
                    style={{ background: 'rgba(11,46,99,0.08)', border: '1px solid rgba(11,46,99,0.12)' }}
                  >
                    {dest}
                  </span>
                ))}
